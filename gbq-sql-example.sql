-- сквозной запрос для распределения выданных заявок на потреб. кредит по последнему источнику перехода
-- таким образом, можно оценить источники не только с точки зрения заявок, но и конечной выдачи кредита  
...
SELECT
  ...
  totalCreditsSum
FROM(
	-- собираем данные в разрезах: дата, источник, регион
SELECT
  date,
  channel,
  region,
...
FROM(
  SELECT
      ...
      site_leads.source,
      site_leads.medium,
      -- сделаем специальную группировку для пары source/medium (по просьбе маркетологов) 
      CASE
